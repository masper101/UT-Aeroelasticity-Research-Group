...
for k = 1:length(SortedData.Fx_inner)
    ...
    ns = sqrt(length(SortedData.instRPM{k}));     %number of samples in each cell
    ...
    RevData.avg_Ttot{k} = RevData.avg_Fz_outer{k} + RevData.avg_Fz_inner{k};
    RevData.avg_Qtot{k} = RevData.avg_Mz_outer{k} + RevData.avg_Mz_inner{k};
    RevData.avg_Ptot{k} = RevData.avg_Qtot{k}.*RevData.avg_instRPM{k}*2*pi/60;
   
    % standard deviation [by azimuthal location]
    ...
    RevData.std_Ttot{k} = nanstd(SortedData.Fz_outer{k}) + nanstd(SortedData.Fz_inner{k});
    RevData.std_Qtot{k} = nanstd(SortedData.Mz_outer{k}) + nanstd(SortedData.Mz_inner{k});
    RevData.std_Ptot{k} = sqrt((RevData.std_Qtot{k}.*RevData.avg_instRPM{k}*2*pi/60).^2 + ...
        (RevData.avg_Qtot{k}.*RevData.err_instRPM{k}).^2);
    ...
    RevData.ms_Ttot{k} = RevData.ms_Fz_outer{k} + RevData.ms_Fz_inner{k};
    RevData.ms_Qtot{k} = RevData.ms_Mz_outer{k} + RevData.ms_Mz_inner{k};
    RevData.ms_Ptot{k} = RevData.ms_Qtot{k}.*RevData.ms_instRPM{k}(2:end)*2*pi/60;
    
    %standard deviation of the revolution
    F_b = 8; M_b = 4.8; I_b = 1.5; %bias errors [N], [N-m], [A]
    RevData.err_ms_curr1{k} = 1.96 * sqrt(nanstd(SortedData.curr1{k}').^2 + I_b^2)./sqrt(length(SortedData.curr1{k}'));
    
    RevData.err_ms_curr2{k} = 1.96 * sqrt(nanstd(SortedData.curr2{k}').^2 + I_b^2)./sqrt(length(SortedData.curr2{k}'));
    
    RevData.err_ms_instRPM{k} = 1.96 * nanstd(SortedData.instRPM{k}')./sqrt(length(SortedData.instRPM{k}'));
    
    RevData.err_ms_Ttot{k} = 1.96 * (sqrt(nanstd(SortedData.Fz_outer{k}').^2 + F_b^2) + ...
        sqrt(nanstd(SortedData.Fz_inner{k}').^2 + F_b^2))./sqrt(length(SortedData.Fz_outer{k}));
    
    RevData.err_ms_Qtot{k} = 1.96 * (sqrt(nanstd(SortedData.Mz_outer{k}').^2 + M_b^2) + ...
        sqrt(nanstd(SortedData.Mz_inner{k}').^2 + M_b^2))./sqrt(length(SortedData.Mz_outer{k}));
    
    RevData.err_ms_Ptot{k} = sqrt((RevData.err_ms_Qtot{k}.*RevData.ms_instRPM{k}(2:end)*2*pi/60).^2 + ...
        (RevData.ms_Qtot{k}.*RevData.err_ms_instRPM{k}(2:end)*2*pi/60).^2);
