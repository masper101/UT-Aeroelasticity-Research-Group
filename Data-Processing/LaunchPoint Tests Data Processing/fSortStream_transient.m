function [StreamData,SortedData,idx_az] = fSortStream_transient(StreamData)
% INPUTS
%     StreamData
...
for k = 1:length(StreamData.names)
    ...
    idx_az{k}(1) = 1; %index position of the revolution in the time vector
    for n = 2:StreamData.nrevs{k} %cut out first rev
        b = SortedData.binsize{k}(n);
        idx_az{k}(n) = idx_az{k}(n-1) + b;
...
%                 StreamData.encoder{k}(count:count-1+b) =[360/b:360/b:360]; 
            ...
            if b > (2^Nbits-1)
                ...
                instRPM = [instRPM instRPM(end)]*60/2/pi; % add one element to get size 1xb and convert to RPM
                % interpolate to azimuth with dpsi = 1/Naz
