function plot_exp(idx,collectives, I1_avg,I2_avg,I1err,I2err,...
...
%Plot data with experimental data only
...
title(strcat('Torque Cell Torque vs Collective (',RPM,' RPM/',string(Nb),...
    '-bladed)'),'interpreter','latex')
xlabel('Collective ($\theta_o$), deg','interpreter','latex')
...
if save_prompt == 'y'
    ...
    dir   = pwd;
    fpath = strcat(dir,strcat('/',filename,'/Experimental')); % directory to save all data

    [status, msg] = mkdir(dir,strcat(filename,'/Experimental')); % make directory
